__authors__ = '1679817, 1679818, 1682347, 1680519'
__group__ = 'DJ.15'
...
class KMeans:
    ...
    def _init_centroids(self):
        ...
        # split the data into k sections and take one point from each
            ...
            for i in range(self.K):
                centroids[i] = self.X[i * group_size]
    ...
    def interClassDistance(self):
        ...
        for i in range(num_classes):
            class_i_points = self.X[self.labels == i]
            
            for j in range(i + 1, num_classes):
                class_j_points = self.X[self.labels == j]
                class_distance = np.mean(np.sqrt(np.sum((class_i_points[:, None] - class_j_points) ** 2, axis=-1)))
                icd += class_distance

        return icd
    
    def fischer(self):
        return self.interClassDistance()/self.withinClassDistance()
